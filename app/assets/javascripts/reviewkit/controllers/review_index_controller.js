...
export default class extends Controller {
  ...
  syncFilterControls() {
    this.filterTargets.forEach((button) => {
      ...
      button.classList.toggle("reviewkit-index-tab--active", active)
      button.setAttribute("aria-selected", active ? "true" : "false")
      button.tabIndex = active ? 0 : -1

      const count = button.querySelector(".reviewkit-index-tab__count")
      if (count) {
        count.classList.toggle("reviewkit-index-tab__count--active", active)
      }
    })
