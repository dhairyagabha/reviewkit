...
module Changeset
  module DiffHelper
    def changeset_document_badge_class(document)
    ...
    def changeset_status_pill_class(review_or_status)
      status = review_or_status.respond_to?(:status) ? review_or_status.status : review_or_status.to_s

      case status
      when "approved"
        "changeset-status-pill changeset-status-pill--approved"
      when "rejected"
        "changeset-status-pill changeset-status-pill--rejected"
      when "closed"
        "changeset-status-pill changeset-status-pill--closed"
      when "resolved"
        "changeset-status-pill changeset-status-pill--resolved"
      when "outdated"
        "changeset-status-pill changeset-status-pill--outdated"
      when "draft"
        "changeset-status-pill changeset-status-pill--draft"
      else
        "changeset-status-pill"
      end
    end

    def changeset_diff_row_class(row)
      "changeset-line changeset-line--#{row.fetch("kind")}"
    end

    def changeset_unified_group_rows(row, document: nil)
      inline_changes =
        if changeset_inline_source_kind(row) == "changed"
          changeset_inline_changes(row, document: document)
        else
          row["inline_changes"]
        end
      ...
      when "changed"
        [
          changeset_unified_row_payload(row, side: "old", kind: "removed", line: row["old_line"], text: row["old_text"], inline_changes: inline_changes),
          changeset_unified_row_payload(row, side: "new", kind: "added", line: row["new_line"], text: row["new_text"], inline_changes: inline_changes)
        ]
      when "removed"
        [ changeset_unified_row_payload(row, side: "old", kind: "removed", line: row["old_line"], text: row["old_text"], inline_changes: inline_changes) ]
      when "added"
        ...
        [ changeset_unified_row_payload(row, side: "context", kind: "context", line: row["new_line"] || row["old_line"], text: row["new_text"].presence || row["old_text"], inline_changes: inline_changes) ]
      end
    end

    def changeset_inline_ranges(row, side:, document: nil)
      return [] unless changeset_inline_source_kind(row) == "changed"

      normalized_side = side.to_s
      return [] unless %w[old new].include?(normalized_side)

      Array(changeset_inline_changes(row, document: document)[normalized_side])
    end

    def changeset_highlight_line(text, language, inline_ranges: nil, inline_side: nil)
      ...
      rendered_html = changeset_apply_inline_ranges(highlighted_html, inline_ranges, inline_side)

      content_tag(:span, rendered_html, class: "highlight")
    end

    def changeset_line_number(value)
      value.presence || "&nbsp;".html_safe
    end

    def changeset_render_comment_body(comment)
      simple_format(h(comment.body), class: "changeset-comment-body")
    end

    private

    def changeset_unified_row_payload(row, side:, kind:, line:, text:, inline_changes:)
      {
        "kind" => kind,
    ...
    def changeset_inline_source_kind(row)
      row["source_kind"].presence || row["kind"].to_s
    end

    def changeset_inline_changes(row, document: nil)
      row["inline_changes"] ||= begin
        context = changeset_intraline_context(document)
        old_text = row["old_text"].to_s
        new_text = row["new_text"].to_s

        if Changeset::Diffs::IntralineBudget.allow?(
          old_text: old_text,
          new_text: new_text,
          review_document_count: context[:review_document_count],
          changed_row_count: context[:changed_row_count]
        )
          Changeset::Diffs::IntralineDiff.call(old_text:, new_text:)
        else
          changeset_empty_inline_changes
        end
      end
    end

    def changeset_intraline_context(document)
      return {} if document.blank?

      document.instance_variable_get(:@changeset_intraline_context) || begin
        context = {
          changed_row_count: document.diff_rows.count { |row| row["kind"] == "changed" },
          review_document_count: document.review&.documents&.size
        }
        document.instance_variable_set(:@changeset_intraline_context, context)
      end
    end

    def changeset_empty_inline_changes
      { "old" => [], "new" => [] }
    end

    def changeset_apply_inline_ranges(highlighted_html, inline_ranges, inline_side)
      normalized_side = inline_side.to_s
      normalized_ranges = changeset_merge_inline_ranges(Array(inline_ranges))
      ...
      fragment.xpath(".//text()").each do |node|
        ...
        node_ranges.each do |range|
          ...
          wrapper["class"] = "changeset-inline-change changeset-inline-change--#{normalized_side}"
    ...
    def changeset_merge_inline_ranges(ranges)
      ranges
        .map do |range|
          {
