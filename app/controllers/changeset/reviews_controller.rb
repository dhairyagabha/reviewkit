...
module Changeset
  class ReviewsController < ApplicationController
    ...
    def index
      ...
      authorize_changeset!(:index, Review)
    end

    def show
      authorize_changeset!(:show, @review)
    end

    def edit
      authorize_changeset!(:edit_review, @review)
      @editing_review = true
      render :show
    end

    def update
      authorize_changeset!(:update_review, @review)
    ...
    def approve
      authorize_changeset!(:approve, @review)
      transition_review!(:approve!, :notice, "Review approved.")
    end

    def reject
      authorize_changeset!(:reject, @review)
      transition_review!(:reject!, :alert, "Review rejected.")
    end

    def destroy
      authorize_changeset!(:destroy, @review)
      @review.destroy!
