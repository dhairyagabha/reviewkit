...
RSpec.describe Reviewkit::Document, type: :model do
  describe "status and diff cache" do
    it "marks a document as added when only new content exists" do
      document = build(:reviewkit_document, old_content: "", new_content: "const total = 2;\n")
    ...
    it "marks a document as removed when only old content exists" do
      document = build(:reviewkit_document, old_content: "const total = 1;\n", new_content: "")
  ...
  describe "validations" do
    it "rejects non-hash metadata" do
      document = build(:reviewkit_document, metadata: "invalid")
    ...
    it "enforces unique paths within a review" do
      ...
      create(:reviewkit_document, review:, path: "rules/cart-tracking.json")
      duplicate = build(:reviewkit_document, review:, path: "rules/cart-tracking.json")
