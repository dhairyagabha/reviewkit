...
RSpec.describe Changeset::ReviewThread, type: :model do
  it "inherits the review from the document" do
    document = create(:changeset_document)
    thread = build(:changeset_review_thread, review: nil, document: document)
  ...
  it "requires an anchor line that matches the chosen side" do
    thread = build(:changeset_review_thread, side: "old", new_line: 2)
    thread.old_line = nil
