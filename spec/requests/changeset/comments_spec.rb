...
RSpec.describe "Changeset comments", type: :request do
  ...
  it "deletes the last comment by removing the entire thread bucket" do
    thread = create(:changeset_review_thread)
    comment = create(:changeset_comment, review_thread: thread, author: reviewer, body: "Remove this thread.")

    expect do
      delete "/changeset/review_threads/#{thread.id}/comments/#{comment.id}",
             headers: turbo_stream_headers
