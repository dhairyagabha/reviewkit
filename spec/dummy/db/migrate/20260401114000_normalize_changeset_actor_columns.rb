...
class NormalizeChangesetActorColumns < ActiveRecord::Migration[8.1]
  def up
    normalize_polymorphic_actor(:changeset_reviews, :creator, legacy_column: :creator_gid)
    normalize_polymorphic_actor(:changeset_review_threads, :resolved_by, legacy_column: :resolved_by_gid)
    normalize_polymorphic_actor(:changeset_comments, :author, legacy_column: :author_gid)
  end
